        ...
        const questions = await Promise.all(
            questionIds.map(async (q: { id_question: number }) => {
                const sqlQuestionDetails = `
                    SELECT question, explanation, answer2, answer3, correct_answer 
                    FROM questions 
                    WHERE id = ?;
                `;
                ...
                const { question, explanation, answer2, answer3, correct_answer } = questionDetails[0];
                ...
                const answersQuery = `
                    ...
                    WHERE id IN (?, ?, ?);
                `;
                const answers = await db.getAllAsync(answersQuery, [answer2, answer3, correct_answer]);
